...
Everyone interacting in the mu_diff project's code base,
...
Report bugs at https://github.com/TejasAvinashShetty/mu_diff/issues.
...
The best way to send feedback is to file an issue at https://github.com/TejasAvinashShetty/mu_diff/issues.
...
3. Check https://travis-ci.org/TejasAvinashShetty/mu_diff/pull_requests
...
Ready to contribute? Follow `Aaron Meurer's Git Workflow Notes`_ (with ``TejasAvinashShetty/mu_diff`` instead of ``sympy/sympy``)

In short,

1. Clone the repository from ``git@github.com:TejasAvinashShetty/mu_diff.git``
...
mu_diff uses a simple branching model where all
...
mu_diff includes a full test-suite using pytest_.
We strive for a `test coverage`_ above 90%.


From a checkout of the ``mu_diff`` repository  you can use
...
.. _test coverage: https://coveralls.io/github/TejasAvinashShetty/mu_diff?branch=master
...
mu_diff could always use more documentation, whether
...
At any point, from a checkout of the ``mu_diff``
...
.. doctest::
    ...
    >>> import mu_diff
    >>> mu_diff.__version__   # doctest: +SKIP
...
https://github.com/TejasAvinashShetty/mu_diff/releases.
